...
def train():
    # load parameters
    start_epoch = 0
    if cfg.RESUME: #检查模型生成路径中有没有上次生成好的模型，加载上次最后一个模型，接着这个开始训练，而不用重头开始训练
        ...
        if len(saved_models) != 0:
            ...
            state_dict = torch.load(loadckpt, map_location=map_location)#加载上次的最后模型，从这个模型开始训练
    ...
    for epoch_idx in range(start_epoch, cfg.TRAIN.EPOCHS):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            loss, scalar_outputs = train_sample(sample)#模型输入为(batchsize； 窗口视图数，即论文里的N；然后是图像三个元素C，W，H）
            if is_main_process():
                logger.info(
