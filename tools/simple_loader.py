...
class ScanNetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, id):
        ...
        print(id1)
        str1 = format(id1, '06d')
        cam_pose=npread.txt_read(os.path.join(self.data_path, self.scene, "pose", str1 + ".txt"))
        #cam_pose = np.loadtxt(os.path.join(self.data_path, self.scene, "pose", str1 + ".txt"), delimiter=' ')

        # Read depth image and camera pose
        depth_im = cv2.imread(os.path.join(self.data_path, self.scene, "depth", str1 + ".png"), -1).astype(
            np.float32)
        depth_im /= 1000.  # depth is saved in 16-bit PNG in millimeters
        depth_im[depth_im > self.max_depth] = 0

        # Read RGB image
        color_image = cv2.cvtColor(cv2.imread(os.path.join(self.data_path, self.scene, "color", str1+ ".jpg")),
                                   cv2.COLOR_BGR2RGB)
