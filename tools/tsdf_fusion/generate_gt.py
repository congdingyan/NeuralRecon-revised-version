...
# python tools/tsdf_fusion/generate_gt.py --data_path '/home/cdy/桌面/NeuralRecon-master/data/data4' --save_name all_tsdf_9
# python tools/tsdf_fusion/generate_gt.py --test --data_path '/home/cdy/桌面/NeuralRecon-master/data/data4' --save_name all_tsdf_9

def parse_args():
    parser = argparse.ArgumentParser(description='Fuse ground truth tsdf')
    parser.add_argument("--dataset", default='scannet')
    parser.add_argument("--data_path", metavar="DIR",
                        help="path to raw dataset", default='/home/cdy/桌面/NeuralRecon-master/data/data4')
...
#    a=npread.txt_read('/home/cdy/桌面/NeuralRecon-master/000000.txt')
    ray.init(num_cpus=all_proc * (args.num_workers + 1), num_gpus=args.n_gpu)

    if args.dataset == 'scannet':
        if not args.test:
            args.data_path = os.path.join(args.data_path, 'scannet')
        else:
    ...
    #dirworks=['scene0000_00', 'scene0000_01', 'scene0000_02']
    results = ray.get(ray_worker_ids)

    #process_with_single_worker(args, dirworks)
    if args.dataset == 'scannet':
        generate_pkl(args)
