...
class ScanNetDataset(Dataset):
    ...
    def read_cam_file(self, filepath, vid):
        id1 = vid * 100
        str1 = format(id1, '06d')
        intrinsics = np.loadtxt(os.path.join(filepath, 'intrinsics_color.txt'), delimiter=' ')[:3, :3]
        intrinsics = intrinsics.astype(np.float32)
        extrinsics = np.loadtxt(os.path.join(filepath, 'pose', str1 + '.txt'))
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(meta['image_ids']):
            ...
            imgs.append(
                self.read_img(
                    os.path.join(self.datapath, self.source_path, meta['scene'], 'color', str1 + '.jpg')))

            depth.append(
                self.read_depth(
                    os.path.join(self.datapath, self.source_path, meta['scene'], 'depth', str1 + '.png'))
            )
